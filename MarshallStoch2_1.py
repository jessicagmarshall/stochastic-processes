...
PM = np.zeros((SNR.size, num_thresh))
point = np.zeros(SNR.size)


for power in range(0, SNR.size):
    for i in range(0, num_trials):
        MAP_temp[i] = np.exp(((-np.square(Y[i]))/(2*np.square(sigma_2))) + ((np.square(Y[i] - SNR[power]))/(2*np.square(sigma_2))))     #MAP statistic
    ...
    for j in range(0, num_thresh):
        ...
        PM[power, j] = np.sum(np.logical_and(event, np.logical_not(detect)))/np.sum(event)
